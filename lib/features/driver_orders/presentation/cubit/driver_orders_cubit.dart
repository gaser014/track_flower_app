...
class DriverOrdersCubit
  ...
  DriverOrdersCubit({
    ...
    _listenForOrderStatusPush();
  }

  /// Subscription to customer-triggered order-status pushes (e.g. delivery
  /// confirmation) delivered via FCM.
  ...
  /// Listen for order-status pushes from the customer app and reflect them in
  /// the driver's active order and orders list.
  void _listenForOrderStatusPush() {
  ...
  Future<void> _advanceOrder() async {
    ...
    // The driver can only advance the order up to "arrived". Marking the order
    // as "delivered" (and beyond) is exclusively the customer's action from
    // their own app, so we never transition past "arrived" here.
    ...
    // Accepting the leg to the store is backed by the "start order" API call;
    // every other step just advances locally and mirrors to Firebase so the
    // customer app sees the new status live.
