...
void main() {
  ...
  setUp(() {
    ...
    useCase = GetPendingOrdersUseCase(mockRepository);
  });

  const tParams = PaginationParams(page: 1, limit: 20);
  final tPage = buildOrdersPage([buildOrder(status: OrderStatus.pending)]);

  test('should return pending orders page from repository', () async {
    when(
    ...
    verify(mockRepository.getPendingOrders(tParams)).called(1);
  });

  test('should return error when repository fails', () async {
    final tException = Exception('Failed to load pending orders');
    when(
      mockRepository.getPendingOrders(any),
